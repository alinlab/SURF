...
class Workspace:
    ...
    def setup(self):
        ...
        self.reward_model = RewardModelSemi(
            ...
            inv_label_ratio=self.cfg.inv_label_ratio,
            threshold_u=self.cfg.threshold_u,
    ...
    def learn_reward(self, first_flag=0):
        ...
        if self.labeled_feedback > 0:
            # update reward
            dataset_size = min(self.labeled_feedback, self.reward_model.capacity)
            num_iters = self.cfg.reward_update*int(np.ceil(dataset_size/self.reward_model.train_batch_size))
            if self.cfg.label_margin > 0:
                raise NotImplementedError
                # total_acc, u_total_acc, u_total_select = self.reward_model.semi_train_soft_reward(num_iters)
            else:
                total_acc, u_total_acc, u_total_select = self.reward_model.semi_train_reward(num_iters)
            total_acc, u_total_acc, u_total_select = np.mean(total_acc), np.mean(u_total_acc), np.mean(u_total_select)

            print('[reward learning] train_acc (labeled): ' + str(total_acc))
            # print('[reward learning] train_acc / select_ratio (unlabeled): ' + str(u_total_acc) + ' / '+ str(u_total_select))
...
@hydra.main(config_path="cfgs", config_name="config_pebble_semi")
def main(cfg):
    root_dir = Path.cwd()
